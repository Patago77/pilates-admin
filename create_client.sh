...
SCHEMA_PATH="/root/estudio-pilates/docs/sql/studio_schema.sql"
if [[ ! -f "${SCHEMA_PATH}" ]]; then
  echo "❌ No existe schema: ${SCHEMA_PATH}"
  exit 1
fi

# Genera contraseña y hash
...
# 4) Crear tablas del estudio (students, payments, gastos)
sudo mysql -e "USE \`${DB_NAME}\`; SOURCE ${SCHEMA_PATH};"
